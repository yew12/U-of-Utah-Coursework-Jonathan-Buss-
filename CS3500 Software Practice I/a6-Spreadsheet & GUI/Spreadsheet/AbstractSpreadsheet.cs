...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        protected IEnumerable<String> GetCellsToRecalculate(ISet<String> names)
        {
            ...
            // go through each of the cells in the set. 
            foreach (String name in names)
            {
                // if we havent visited that cell, then visit and recalculate
                if (!visited.Contains(name))
                {
                    Visit(name, name, visited, changed);
                }
            }
            // return the reordered list of cells
            return changed;
        }
        ...
        /// 
        /// Enumerates through our dependents and marks if they have been visited
        /// </summary>
        private void Visit(String start, String name, ISet<String> visited, LinkedList<String> changed)
        {
            // first add name to visited hashset so we don't visit twice. 
            visited.Add(name);
            // then go through and get all the direct dependents of cell needed to be recalculated
            foreach (String n in GetDirectDependents(name))
            {
                // we at any point see the cell that we started with, throw CircularException 
                if (n.Equals(start))
                {
                    throw new CircularException();
                }
                // otherwise, if we haven't visited, go back and visit
        ...
        // ****************************************** \\ 
        // EVERYTHING BELOW ARE SPECIFICATIONS FOR A5
        // ****************************************** \\ 
